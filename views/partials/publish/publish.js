window.devportal.partials.publish = function($scope,$rootScope, $http, $dev, $state, $rootScope, $stateParams, $auth, $objectstore,$helpers){
	...
	$scope.publish = function(){
		...
		$dev.project().publish($scope.appKey,mObj).success(function(){
			...
			// this is where we have to show a small toast saying that app was succesfully published !
		}).error(function(){
			$dev.dialog().alert ();
			// this is where we have to show an error modal when the app is not published properly !
		});
	}
